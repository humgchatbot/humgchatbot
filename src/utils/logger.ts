...
const logPair = async (id1: string, id2: string): Promise<void> => {
  ...
  data += "&entry."+config.POST_LOG_NAME1+"=" + info1.name;
  data += "&entry."+config.POST_LOG_NAME2+"=" + info2.name;
  
  let data = `entry.${config.POST_LOG_NAME1}=${id1}&entry.${config.POST_LOG_NAME2}=${id2}`;

  const info1.name = await fb.getUserData(id1);
  const info2.name = await fb.getUserData(id2);
